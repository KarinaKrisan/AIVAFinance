...
import { LayoutDashboard, PieChart, Bell, LogOut, Wallet, User, Copy } from 'lucide-react';
import { FinanceContext } from '../context/FinanceContext';
import { auth } from '../firebase'; // Importação necessária para o Logout funcionar
...
export default function Sidebar() {
  ...
  // Função de Logout Real (Limpa Firebase + Redireciona)
  const handleLogout = async () => {
    try {
      await auth.signOut();
      localStorage.clear();
      window.location.href = "/"; // Força o recarregamento para a tela de login
    } catch (error) {
      console.error("Erro ao sair:", error);
    }
  };

  // Função para copiar o ID clicando nele
  const copyToClipboard = () => {
    if (user?.uid) {
      navigator.clipboard.writeText(user.uid);
      alert("ID copiado! Cole no Telegram: /conectar " + user.uid);
    }
  };
    ...
    <div className="w-64 bg-white border-r border-gray-100 flex flex-col h-full hidden md:flex">
      ...
      <div className="p-8 border-b border-gray-50">
        <div className="flex flex-col items-center text-center">
            ...
            <p className="text-xs text-gray-400 truncate w-full mb-2">{user?.email}</p>
            
            {/* NOVO: MOSTRADOR DE ID PARA O BOT */}
            <div 
              onClick={copyToClipboard}
              className="flex items-center gap-2 bg-gray-100 px-3 py-1 rounded text-[10px] text-gray-500 cursor-pointer hover:bg-emerald-100 hover:text-emerald-700 transition"
              title="Clique para copiar seu ID"
            >
              <span className="truncate max-w-[100px] font-mono">
                ID: {user?.uid || "Carregando..."}
              </span>
              <Copy size={10} />
            </div>
